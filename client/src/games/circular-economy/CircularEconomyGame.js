...
const CircularEconomyGame = () => {
	...
	const result = () => {
		...
		if (now.length > future.length && now.length > never.length) {
			...
			result =
				'Great choices, you’ve selected a mix of acting now and making changes in the future.'
		} else if (never.length > now.length && never.length > future.length) {
			result =
				'It looks like you’re happy with the way things are. This might cause some waste issues. You need to make some more ambitious choices before we can award you the badge.'
		} else {
			result =
				'Great choices, you’ve selected a mix of acting now and making changes in the future.'
		}
	...
	const [circularEconomyResult, setCircularEconomyResult] = useState(
		circularEconomyText || ''
	)
	...
	useEffect(() => {
		window.localStorage.setItem(
			'result4',
			JSON.stringify(circularEconomyResult)
		)
			...
			<Route path='/circular-economy/result'>
				<Result
					...
					badge={
						circularEconomyText !==
						'It looks like you’re happy with the way things are. This might cause some waste issues. You need to make some more ambitious choices before we can award you the badge.'
							? "You've earned the SHARING ECONOMY BADGE"
							: null
					}
